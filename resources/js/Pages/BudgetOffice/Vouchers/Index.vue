...
useBreadcrumb([{ label: 'Budget Office' }, { label: 'Vouchers' }])

const props = defineProps({
  applications: { type: Object, default: () => ({}) },
  filters: { type: Object, default: () => ({}) },
  tab: { type: String, default: 'budget_checking' },
...
const tabIndex = ['budget_checking', 'approved', 'voucher_on_hold'].indexOf(props.tab)
...
usePolling(
  route('budget-office.vouchers.poll'),
  pollParams,
  (data) => {
...
function applyFilters() {
  router.get(route('budget-office.vouchers.index'), {
    tab: props.tab,
    search: search.value || null,
    category: category.value || null,
    page: 1,
...
function onTabChange(event) {
  const tabValues = ['budget_checking', 'approved', 'voucher_on_hold']
  router.get(route('budget-office.vouchers.index'), {
...
<template>
  <Head title="Budget Office - Vouchers" />
    ...
    <div class="col-span-12">
      <div class="card">
        <div class="flex items-center justify-between mb-6">
          ...
          <AppExportButton
            :url="route('budget-office.vouchers.export')"
            :params="exportParams"
          />
        </div>
            ...
            <Deferred data="applications">
              <DataTable :value="toRaw(tableData)" striped-rows class="w-full">
                ...
                <Column header="Actions" style="width: 6rem">
                  <template #body="{ data }">
                    <Button icon="pi pi-receipt" severity="info" text rounded size="small" v-tooltip="'Check voucher'"
                      @click="router.get(route('budget-office.vouchers.show', data.id))" />
                  </template>
                </Column>
                <template #empty>
                  <AppEmptyState icon="pi pi-inbox" message="No vouchers pending budget check" />
                </template>
              </DataTable>
          ...
          <TabPanel header="Approved">
            <Deferred data="applications">
              <DataTable :value="toRaw(tableData)" striped-rows class="w-full">
                ...
                <Column header="Actions" style="width: 6rem">
                  <template #body="{ data }">
                    <Button icon="pi pi-eye" severity="info" text rounded size="small" v-tooltip="'View'"
                      @click="router.get(route('budget-office.vouchers.show', data.id))" />
                  </template>
                </Column>
                <template #empty>
                  <AppEmptyState icon="pi pi-check-circle" message="No approved vouchers yet" />
                </template>
              </DataTable>
          ...
          <TabPanel header="On Hold">
            <Deferred data="applications">
              <DataTable :value="toRaw(tableData)" striped-rows class="w-full">
                ...
                <Column header="Actions" style="width: 6rem">
                  <template #body="{ data }">
                    <Button icon="pi pi-eye" severity="info" text rounded size="small" v-tooltip="'Release hold'"
                      @click="router.get(route('budget-office.vouchers.show', data.id))" />
                  </template>
                </Column>
                <template #empty>
                  <AppEmptyState icon="pi pi-check-circle" message="No vouchers on hold" />
                </template>
              </DataTable>
