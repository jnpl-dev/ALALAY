...
function onTabChange(event) {
  ...
  router.get(route('accountant.vouchers.index'), { tab: tabValues[event.index], search: searchQuery.value || null, category: categoryFilter.value || null }, { replace: true })
}
        ...
        <TabView :activeIndex="tabIndex" @tab-change="onTabChange">
          ...
          <TabPanel header="Returned">
            ...
            <DataTable :value="toRaw(tableData)" striped-rows class="w-full">
              ...
              <Column field="status" header="Status" sortable>
                <template #body="{ data }">
                  <Tag value="Returned" severity="warn" />
                </template>
              </Column>
              <Column field="created_at" header="Submitted" sortable>
                <template #body="{ data }">
