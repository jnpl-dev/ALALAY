        ...
        <div class="card mb-0">
          <div class="flex justify-between mb-4">
            <div>
              ...
              <div class="text-surface-900 font-medium text-xl">{{ 'PHP ' + Number(disbursedThisMonth).toLocaleString() }}</div>
            </div>
          ...
          <table v-if="recentTransactions.length" class="w-full">
            ...
            <tbody>
              <tr v-for="txn in recentTransactions" :key="txn.id" class="border-t border-surface">
                ...
                <td class="py-2 text-surface-900">{{ 'PHP ' + Number(txn.amount).toLocaleString() }}</td>
              </tr>
            </tbody>
          </table>
          <div v-else class="flex flex-col items-center justify-center py-8 text-muted-color">
