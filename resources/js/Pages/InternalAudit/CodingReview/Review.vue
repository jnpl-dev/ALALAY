...
useBreadcrumb([{ label: 'Internal Audit' }, { label: 'Coding Review' }, { label: 'Review' }])
...
const canReview = computed(() => props.application.status === 'internal_audit_review')
...
function confirmApprove() {
  confirm.require({
    message: 'Approve this coding and move the application to Voucher Creation?',
    ...
    accept: () => {
      ...
      form.post(route('internal-audit.applications.approve', props.application.id), {
        preserveState: true,
        preserveScroll: true,
        onError: () => {
...
function confirmReturn() {
  confirm.require({
    message: 'Return this coding to AICS for re-coding?',
...
function onReturnConfirmed(data) {
  router.post(route('internal-audit.applications.return', props.application.id), data, {
    preserveState: true,
    preserveScroll: true,
...
<template>
  <Head :title="'Coding Review - ' + application.reference_code" />
  ...
  <div class="grid grid-cols-12 gap-8">
      ...
      <div class="card">
        <div class="flex items-center justify-between mb-4">
          ...
          <Button icon="pi pi-arrow-left" label="Back" severity="secondary" text
            @click="router.get(route('internal-audit.applications.index'))" />
        </div>
        ...
        <div>
          ...
          <dl v-if="application.assistance_code" class="grid grid-cols-2 gap-x-6 gap-y-3 text-sm">
            <div>
              <dt class="text-muted-color">Code Type</dt>
              <dd class="font-medium text-surface-900">{{ application.assistance_code.code_type }}</dd>
            </div>
            <div>
              <dt class="text-muted-color">Amount</dt>
              <dd class="font-medium text-surface-900">{{ formatCurrency(application.assistance_code.amount) }}</dd>
            </div>
            <div v-if="application.assistance_code.description">
              <dt class="text-muted-color">Description</dt>
              <dd class="font-medium text-surface-900">{{ application.assistance_code.description }}</dd>
            </div>
            <div v-if="application.assistance_code.default_amount">
              <dt class="text-muted-color">Default Amount</dt>
              <dd class="font-medium text-surface-900">{{ formatCurrency(application.assistance_code.default_amount) }}</dd>
            </div>
            <div>
              <dt class="text-muted-color">Assigned by</dt>
              <dd class="font-medium text-surface-900">{{ application.assistance_code.assigned_by }}</dd>
            </div>
            <div v-if="application.assistance_code.assigned_at">
              <dt class="text-muted-color">Assigned on</dt>
              <dd class="font-medium text-surface-900">{{ application.assistance_code.assigned_at }}</dd>
            </div>
          </dl>
        ...
        <template v-if="canReview">
          ...
          <div class="flex gap-3">
            <Button label="Approve Coding" icon="pi pi-check" severity="success" @click="confirmApprove"
              :loading="form.processing || approveLoading" class="active:scale-[0.98] transition-transform" />
            <Button label="Return" icon="pi pi-undo" severity="warn" @click="confirmReturn"
              :loading="returnLoading" class="active:scale-[0.98] transition-transform" />
          </div>
        </template>
      </div>
    </div>
    ...
    <ReturnModal v-model:visible="showReturnModal" @confirmed="onReturnConfirmed" />
  </div>
</template>
