...
useBreadcrumb([{ label: 'Internal Audit' }, { label: 'Coding Review' }])
...
const tabIndex = ['pending', 'audited'].indexOf(props.tab)
...
usePolling(
  route('internal-audit.applications.poll'),
  pollParams,
  (data) => {
...
function applyFilters() {
  router.get(route('internal-audit.applications.index'), {
    tab: props.tab,
    search: search.value || null,
    category: category.value || null,
    from: formatDateParam(from.value),
...
function onTabChange(event) {
  const tabValues = ['pending', 'audited']
  router.get(route('internal-audit.applications.index'), {
...
<template>
  <Head title="Coding Review" />

  <div class="grid grid-cols-12 gap-8">
    <div class="col-span-12">
      <div class="card">
        <div class="flex items-center justify-between mb-6">
          <div class="font-semibold text-xl">Coding Review</div>
          <AppExportButton
            :url="route('internal-audit.applications.export')"
            :params="exportParams"
          />
        </div>
            ...
            <Deferred data="applications">
              <DataTable :value="toRaw(tableData)" striped-rows class="w-full">
                ...
                <Column header="Actions" style="width: 6rem">
                  <template #body="{ data }">
                    <Button icon="pi pi-check-circle" severity="info" text rounded size="small"
                      v-tooltip="'Review coding'"
                      @click="router.get(route('internal-audit.applications.show', data.id))" />
                  </template>
                </Column>
                <template #empty>
                  <AppEmptyState icon="pi pi-inbox" message="No applications pending coding review" />
                </template>
              </DataTable>
                ...
                <Column header="Actions" style="width: 6rem">
                  <template #body="{ data }">
                    <Button icon="pi pi-eye" severity="info" text rounded size="small"
                      v-tooltip="'View coding review'"
                      @click="router.get(route('internal-audit.applications.show', data.id))" />
                  </template>
                </Column>
                <template #empty>
                  <AppEmptyState icon="pi pi-check-circle" message="No audited applications" />
                </template>
              </DataTable>
