...
const handleScroll = () => {
  ...
  const offsets = sections.map(s => {
    ...
    if (!el) return { id: s.id, offset: Infinity, label: '' }
    const rect = el.getBoundingClientRect()
    return { id: s.id, offset: Math.abs(rect.top), label: '' }
  })
    ...
    <footer class="text-white bg-emerald-950">
      <div class="px-4 py-12 mx-auto max-w-7xl sm:px-6 lg:px-8">
        <div class="grid grid-cols-1 gap-8 md:grid-cols-4">
          ...
          <div>
            <h4 class="mb-4 text-sm font-semibold tracking-wider uppercase text-emerald-200">{{ $t('footer.quick_links') }}</h4>
            <ul class="space-y-2">
              <li><button @click="scrollToSection('about')" class="text-sm transition-[color,transform] duration-150 text-emerald-300 hover:text-white press-feedback">{{ $t('nav.about') }}</button></li>
              <li><button @click="scrollToSection('programs')" class="text-sm transition-[color,transform] duration-150 text-emerald-300 hover:text-white press-feedback">{{ $t('nav.programs') }}</button></li>
              <li><button @click="scrollToSection('how-it-works')" class="text-sm transition-[color,transform] duration-150 text-emerald-300 hover:text-white press-feedback">{{ $t('nav.how_it_works') }}</button></li>
              <li><button @click="scrollToSection('faqs')" class="text-sm transition-[color,transform] duration-150 text-emerald-300 hover:text-white press-feedback">{{ $t('nav.faqs') }}</button></li>
              <li><button @click="scrollToSection('contact')" class="text-sm transition-[color,transform] duration-150 text-emerald-300 hover:text-white press-feedback">{{ $t('nav.contact') }}</button></li>
            </ul>
          </div>
          <div>
            <h4 class="mb-4 text-sm font-semibold tracking-wider uppercase text-emerald-200">{{ $t('footer.services') }}</h4>
            <ul class="space-y-2">
              <li><Link :href="applyUrl" class="text-sm transition-[color,transform] duration-150 text-emerald-300 hover:text-white press-feedback">{{ $t('footer.apply') }}</Link></li>
              <li><Link :href="trackUrl" class="text-sm transition-[color,transform] duration-150 text-emerald-300 hover:text-white press-feedback">{{ $t('footer.track') }}</Link></li>
            </ul>
            <h4 class="mt-6 mb-4 text-sm font-semibold tracking-wider uppercase text-emerald-200">{{ $t('footer.agency_partners') }}</h4>
